...
public class CandidatureManager { //
     ...
     */
    @POST
    @Path("/{uid}/qualifierCand") /**C/:sid/candidatures/qualifierCand?idSUV=:idSessionUV&idCand=:idCandidature&statCand="accepter"*/
    @Consumes ({MediaType.APPLICATION_JSON})
    @Produces("text/plain")
        ...
        if (this.controleur.exist(sid)) {
            ...
            if (this.controleur.estGestionnaire(idA)) {
                ...
                if(this.controleur.existCandidature(idCandidature)){
                 ...
                 else {message = " The specified candidature doesn't exist";}
            }
            else {message = " You are not allowed to modified candidature";}
        }
        return message;
